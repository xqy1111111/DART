...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return attn_output, attn_weights, query_states, key_states, value_states # ! debug
...
class Qwen2_5_VLDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Self Attention
        hidden_states, self_attn_weights, query_states, key_states, value_states = self.self_attn(  # ! debug
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.language_model = DART._from_config(config.text_config)
...
class DART(Qwen2_5_VLTextModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        for decoder_layer in self.layers:
            ...
            if (
                DART_config 
                and decoder_layer.self_attn.layer_idx == K 
                and seq_length > 1
            ):
                device = hidden_states.device
                ...
                # pdb.set_trace() 
                # Update causal mask
                if causal_mask_mapping["full_attention"]:
                    causal_mask_mapping = {
                        "full_attention": causal_mask_mapping["full_attention"][:, :, keep_indexs, :][:, :, :, keep_indexs],
                    }
                if self.has_sliding_layers:
                    causal_mask_mapping["sliding_attention"] = causal_mask_mapping["sliding_attention"][:, :, keep_indexs, :][:, :, :, keep_indexs]

                # Update text_position_ids
                if text_position_ids is not None:
                    text_position_ids = text_position_ids[:, keep_indexs]
                else:
                    text_position_ids = cache_position.unsqueeze(0)  # Shape: [1, new_seq_length]

                # Update position_ids and position_embeddings
                position_ids = position_ids[:, :, keep_indexs]  # Shape: [3, batch_size, new_seq_length]
                position_embeddings = self.rotary_emb(hidden_states, position_ids)
    ...
    def get_retained_image_token(self, config: Qwen2_5_VLTextConfig, last_layer_state: torch.Tensor, any_states: torch.Tensor) -> torch.Tensor:
